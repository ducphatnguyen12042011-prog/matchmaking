/**
 * ==========================================
 * PRIMEBLOX MULTIPLAYER SYSTEM V12 - ELITE
 * FULL: AUTO-LB, VOICE LOCK, AUTO-MOVE, NEW STATS
 * ==========================================
 */
...
const CONFIG = {
    ...
    LB_CHANNEL_ID: "1474674662792232981", // ID Kênh BXH tự động
...
async function updateAutoLB() {
    try {
        ...
        const lbText = top.map((u, i) => {
            ...
            return `${medal} \`${u.robloxName.padEnd(14)}\` | **${u.elo}** ELO | ${u.wins}W-${u.losses}L`;
        }).join('\n');
        ...
        const embed = new EmbedBuilder()
            ...
            .setFooter({ text: `Cập nhật tự động mỗi 5 phút • ${new Date().toLocaleString()}` });
...
client.on('ready', async () => {
    console.log(`[SYSTEM] PrimeBlox V12 Online: ${client.user.tag}`);
    client.user.setActivity('Leaderboard Update', { type: ActivityType.Watching });
    
    // Khởi động vòng lặp BXH
    updateAutoLB();
...
client.on('messageCreate', async (msg) => {
    ...
    if (command === 'j') {
        ...
        if (Object.values(queues).some(q => q.players.some(p => p.id === msg.author.id))) return msg.reply("🚫 Bạn đã trong hàng chờ!");

        queues[mode].players.push({ id: msg.author.id, name: rows[0].robloxName, elo: rows[0].elo });
        msg.channel.send({ embeds: [new EmbedBuilder().setDescription(`📥 **${rows[0].robloxName}** đã vào hàng chờ **${mode}**`).setColor(CONFIG.COLOR.INFO)] });
        ...
        if (queues[mode].players.length === queues[mode].limit) {
            ...
            const t1 = players.slice(0, players.length / 2), t2 = players.slice(players.length / 2);

            try {
                // TẠO PHÒNG VOICE VÀ KHÓA
                const createMatchVC = async (name, pList) => {
                    return await msg.guild.channels.create({
                        name: name, type: ChannelType.GuildVoice, parent: CONFIG.CATEGORY_VOICE_ID,
                ...
                msg.channel.send({ content: "@everyone", embeds: [new EmbedBuilder().setTitle(`⚔️ MATCH FOUND | #${mId}`).setImage(CONFIG.BANNER_URL).addFields({ name: `🟦 ${rN[0]}`, value: t1.map(p => p.name).join('\n'), inline: true }, { name: `🟥 ${rN[1]}`, value: t2.map(p => p.name).join('\n'), inline: true }).setColor(CONFIG.COLOR.GOLD)] });
                ...
                const processMove = async (pList, vc) => {
                    for (const p of pList) {
                        ...
                        if (m) {
                            m.send({ embeds: [new EmbedBuilder().setTitle("🛡️ TRẬN ĐẤU SẴN SÀNG!").setDescription(`Phòng: ${vc.url}\nLink VIP: [Tại đây](${CONFIG.VIP_LINK})`).setColor(CONFIG.COLOR.SUCCESS)] }).catch(() => {});
                            if (m.voice.channel) await m.voice.setChannel(vc).catch(() => {});
                        }
                    }
                };
                await processMove(t1, vc1); await processMove(t2, vc2);
    ...
    if (command === 'stats') {
        ...
        const statsEmbed = new EmbedBuilder()
            ...
            .setColor(CONFIG.COLOR.DARK).setFooter({ text: `W: ${r[0].wins} | L: ${r[0].losses}` });
    ...
    if (command === 'win') {
        ...
        if (matchIdx === -1) return msg.reply("❌ ID sai.");
...
client.on('interactionCreate', async (i) => {
    if (i.isButton() && i.customId === 'v_start') {
        ...
        modal.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('r_u').setLabel("Roblox User").setStyle(TextInputStyle.Short)));
